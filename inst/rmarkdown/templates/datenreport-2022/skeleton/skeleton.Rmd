...
Die Studienverlaufsbefragung der RUB wird seit dem Sommersemester 2007 jährlich durchgeführt. Studierende, die die Abschlussarten Staatsexamen oder Magister Theologiae anstreben werden im fünften Fachsemester befragt, Masterstudierende im dritten Fachsemester. Die Studienverlaufsbefragung beinhaltet Fragen zum Studienverlauf, zur Finanzierung des Studiums sowie zur Beurteilung von Studienangeboten und -bedingungen und Problemen im Studium.

#### Absolventinnen- und Absolventenbefragung der RUB

Die Absolventinnen- und Absolventenbefragung der RUB wird seit dem Wintersemester 2007/2008 jährlich durchgeführt. Befragt werden alle Absolventinnen und Absolventen, die vor ein- bis eineinhalb Jahren ihr Studium an der RUB abgeschlossen haben. Die Absolventinnen- und Absolventenbefragung der RUB ist ein Kooperationsprojekt, an welchem sich etwa 60 Hochschulen beteiligen und das vom Institut für angewandte Statistik (istat) in Kassel durchgeführt wird. Neben der rückblickenden Beurteilung der Studienangebote und -bedingungen und der Darstellung der Studienverläufe geben die Ergebnisse der Absolventinnen- und Absolventenbefragung Auskunft über den Übergang in ein weiteres Studium bzw. in den Arbeitsmarkt.

### Hinweise zu den Befragungsdaten im Datenreport

Im Datenreport werden Ergebnisse des Studierendenmonitors der RUB (Studieneingangs- und Studienverlaufsbefragung) und Ergebnisse der Absolventinnen- und Absolventenbefragung der RUB in Abbildungen dargestellt.[^1] Um möglichst differenzierte Ergebnisse auf Studiengangebene  zu erhalten, werden die Befragungsdaten mehrerer Jahrgänge zusammengefasst, die jeweils der Quellenangabe zu entnehmen sind.[^2] Die Ergebnisse des Datenreports beziehen die Studiengänge mit ein, für welche ausreichend Fallzahlen in den Befragungsdaten vorliegen. Dabei werden dann Auswertungen vorgenommen, wenn zu Faktenfragen mit zwei Antwortmöglichkeiten mindestens Antworten von zehn Befragten vorlagen (z.B.: Abschluss des Studiums in der Regelstudienzeit, Antwort: „ja“ oder „nein“). Bei allen anderen Fragen werden Auswertungen durchgeführt, wenn mindestens fünfzehn Befragte geantwortet haben. Die Anzahl der jeweiligen Antworten ist den Abbildungen zu entnehmen (z.B.: n=25).[^3] 

Die Auswertungen erfolgen immer auf der Grundlage des Studienfaches, welches von den Befragten im Fragebogen als erstes Studienfach definiert wurde. Eine genaue Übersicht der in den Datenreports berücksichtigten Fach-Abschluss-Kombinationen inkl. der Fächergruppenzuordnung findet sich im Anhang.

In der Studieneingangsbefragung werden Studierende im ersten Hochschulsemester befragt, so dass hier in gestuften Studiengängen lediglich Auswertungen für das Bachelorstudium vorliegen. Bei der Studienverlaufs- sowie bei der Absolventinnen- und Absolventenbefragung werden bei ausreichend großer Fallzahl auch die Antworten, die sich auf das Masterstudium beziehen, ausgewertet.

Als Vergleichswert werden für alle Befragungen die Ergebnisse auf Fächergruppen nach der Gliederung des statistischen Bundesamtes aggregiert.[^4] Der jeweilige Fächergruppenwert der RUB wird, differenziert nach Abschlussart in den Abbildungen ausgewiesen (abgekürzt z.B.: „FG Bachelor 1-Fach").
...
Dieser Datenreport stellt einige ausgewählte hochschulstatistische Daten in Abbildungen zur Verfügung, die in Tabellenform jederzeit auch aus dem Informationsmanagement-System der RUB abrufbar sind. Die Auswertungen berücksichtigen Studienfälle, d.h. wenn Personen zwei Studienfächer studieren, werden sie in beiden Studienfächern gezählt. Dabei werden Auswertungen vorgenommen, wenn mindestens 20 Fälle vorliegen. Auch die hochschulstatistischen Daten werden anhand der Übersicht der berücksichtigten Fach-Abschluss-Kombinationen im Anhang zusammengestellt. Zudem werden zum Teil mehrere Semester bzw. Studienjahre zusammengefasst. Diese Information ist der Quellenangabe der jeweiligen Abbildung zu entnehmen.

## Informationen zu den Förderungen für die Weiterentwicklung der Lehre

Der Datenreport gibt neben den Daten zu Studierenden und ihrem Studienverlauf auch Auskunft darüber, für welche Lehrprojekte Fakultäten und ZWE im Zeitraum vom Sommersemester 2016 bis zum Sommersemester 2018 eine Förderung aus wettbewerblicher Vergabe erhalten haben.
...
tpl_chunk_1 <- c(
  "\n```{r prep-fig-{{label}}, include = FALSE}", "df_fig <- Datenreport2018::get_figure_df{{func}}",
  "```"
)
...
# tpl_chunk_5 <- c(
#   "\n```{r question-fig{{label}}, results = 'asis', eval = !is.na(df_fig$question_txt[[1]])}",
#   "cat(paste(\"######\", df_fig$question_txt[[1]]))",
#   "```"
# )

tpl <- c(tpl_chunk_1, tpl_chunk_2, tpl_chunk_3, tpl_chunk_4 
       #  ,tpl_chunk_5
         )

src <- purrr::map2(chunk_label, function_parameters, function(i, m) {
  knitr::knit_expand(text = tpl, label = i, func = m)
})
...
degree_programs_path <- system.file(
  ...
  "datenreport-2018",
  "skeleton",
  "degree_programs.xlsx",
  package = "RUBer"
)

degree_programs <- readxl::read_xlsx(
  path = degree_programs_path,
  sheet = "included_programs"
  ) %>%
  mutate(
    row_name = row_number()
    ) %>%
...
footnote_text <- "Im Datenreport Nr. 62 sind fächergruppenübergreifend alle Master of Education Studiengänge berücksichtigt, weshalb der Datenreport mehrfach aufgeführt wird."

rows_footnote <- degree_programs %>%
  filter(head1 == 62) %>%
  pull(row_name)

flextable::flextable(degree_programs, col_keys = c("head1", "head2", "head3")) %>%
  theme_zebra(odd_body=grey_fill) %>%
  footnote(i = rows_footnote, value = as_paragraph(footnote_text), ref_symbols = "*", part = "body") %>%
...
excluded_programs <- readxl::read_xlsx(
  path = degree_programs_path,
  sheet = "excluded_programs"
  ) %>%
  dplyr::select(
    head1 = `Ausschluss-grund`,
    head2 = `Fach-Abschluss-Kombinationen`,
    head3 = `Fächergruppe`
    )
...
flextable::flextable(excluded_programs, col_keys = c("head1", "head2", "head3")) %>%
  theme_zebra(odd_body=grey_fill) %>%
  set_header_labels(head1 = "Ausschluss-grund", head2 = "Fach-Abschluss-Kombinationen", head3 = "Fächergruppe") %>%
  width(j = "head1", width = 1) %>%
  width(j = "head2", width = 3.63) %>%
  width(j = "head3", width = 2.25) %>%
  RUBer::rub_style_flextable()
...
Als Studienabbrecher werden hier Studierende verstanden die an der RUB ein Studium begonnen haben, aber sich ohne einen Abschluss wieder exmatrikuliert haben und die RUB verlassen haben. In diesen Fällen kann es sich sowohl um einen Hochschulwechsler handeln als auch um jemanden der insgesamt das Studium ohne Erfolg beendet hat.
...
Absolventen sind Studierende mit bestandener Abschlussprüfung, die im Berichtssemester mindestens einen Studiengang abgeschlossen haben.
...
> **DR. JUDITH RICKEN**  
> Tel.: 0234 - 32-23187  
E-Mail: [judith.ricken@uv.rub.de](mailto:judith.ricken@uv.rub.de)
